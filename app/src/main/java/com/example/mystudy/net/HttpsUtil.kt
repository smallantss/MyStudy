...
class HttpsUtil {
    ...
    class SSLParams {
        var sSLSocketFactory: SSLSocketFactory by Delegates.notNull()
        var trustManager: X509TrustManager by Delegates.notNull()
    }
    ...
    companion object {
        ...
        private fun getSslSocketFactoryBase(trustManager: X509TrustManager?, bksFile: InputStream?, password: String?,
                                            vararg certificates: InputStream?): SSLParams {
            ...
            return try {
                ...
                //优先使用用户自定义的TrustManager 然后使用默认的TrustManager 否则使用不安全的TrustManager
                manager = trustManager ?: (trustManagers?.let { chooseTrustManager(it) }
                        ?: UnSafeTrustManager)
